...
class FlowCanvasView(context: Context) : View(context) {
    ...
    private fun adaptiveStub(distance: Float): Float =
        (18f + (distance * 0.06f).coerceIn(0f, 24f)).coerceIn(16f, 40f)
    ...
    /**
     * Pure smooth-curve routing (no orthogonal elbows, no path through shapes).
     *
     * Every connector:
     *  1. Leaves the start face along its outward normal (short stub)
     *  2. Curves around the *outside* of both boxes
     *  3. Approaches the end face along its outward normal
     *
     * Properly facing pairs (e.g. bottom of upper → top of lower) use one cubic.
     * All other combinations use a multi-cubic “pipe” that arcs around the side.
     */
    private fun buildDynamicRoutedPath(
        ...
        // Everything else: smooth arc around the outside of both boxes.
        return buildAroundCurve(a, b, start, end, fromSide, toSide, sourceOut, targetOut, stub)
    }
    ...
    private fun isProperlyFacing(
        ...
        return when {
            fromSide == ConnectionSide.BOTTOM && toSide == ConnectionSide.TOP -> acy < bcy
            fromSide == ConnectionSide.TOP && toSide == ConnectionSide.BOTTOM -> acy > bcy
            fromSide == ConnectionSide.RIGHT && toSide == ConnectionSide.LEFT -> acx < bcx
            fromSide == ConnectionSide.LEFT && toSide == ConnectionSide.RIGHT -> acx > bcx
            else -> false
        }
    }

    /**
     * Builds a continuous multi-cubic path that goes around both shapes.
     * Example: top of upper box → bottom of lower box arcs out to the side,
     * down past both boxes, then under the lower box into its bottom face.
     */
    private fun buildAroundCurve(
        ...
        val pad = 36f
        val left = min(a.x, b.x) - pad
        val right = max(a.x + a.width, b.x + b.width) + pad
        val top = min(a.y, b.y) - pad
        val bottom = max(a.y + a.height, b.y + b.height) + pad
        val midX = (start.x + end.x) / 2f
        val midY = (start.y + end.y) / 2f

        // Choose the shorter side to loop around.
        val goLeft = (midX - left) <= (right - midX)
        val sideX = if (goLeft) left else right
        val goUp = (midY - top) <= (bottom - midY)
        val sideY = if (goUp) top else bottom

        // Keyframe points that stay outside both boxes.  Path is then turned
        // into pure cubics so there are no visible elbows.
        val keys = mutableListOf<PointF>()
        keys += start
        keys += sourceOut
        ...
        if (fromSide == toSide) {
            // Same face: push out further, travel parallel, come back.
            when (fromSide) {
                ConnectionSide.TOP -> {
                    val y = min(sourceOut.y, targetOut.y) - 28f
                    keys += PointF(sourceOut.x, y)
                    keys += PointF(targetOut.x, y)
                }
                ConnectionSide.BOTTOM -> {
                    val y = max(sourceOut.y, targetOut.y) + 28f
                    keys += PointF(sourceOut.x, y)
                    keys += PointF(targetOut.x, y)
                }
                ConnectionSide.LEFT -> {
                    val x = min(sourceOut.x, targetOut.x) - 28f
                    keys += PointF(x, sourceOut.y)
                    keys += PointF(x, targetOut.y)
                }
                else -> {
                    val x = max(sourceOut.x, targetOut.x) + 28f
                    keys += PointF(x, sourceOut.y)
                    keys += PointF(x, targetOut.y)
                }
            }
        } else if (!sameAxis) {
            // Adjacent faces: one smooth corner via an outside control point.
            keys += PointF(
                if (fromSide == ConnectionSide.LEFT || fromSide == ConnectionSide.RIGHT) targetOut.x else sourceOut.x,
                if (fromSide == ConnectionSide.TOP || fromSide == ConnectionSide.BOTTOM) targetOut.y else sourceOut.y
            )
        } else {
            // Opposite faces but wrong order (e.g. top→bottom while A is above B):
            // arc around the side of both boxes.
            val vertical = fromSide == ConnectionSide.TOP || fromSide == ConnectionSide.BOTTOM
            if (vertical) {
                keys += PointF(sideX, sourceOut.y)
                keys += PointF(sideX, targetOut.y)
            } else {
                keys += PointF(sourceOut.x, sideY)
                keys += PointF(targetOut.x, sideY)
            }
        }
        ...
        return smoothCubicThrough(keys)
    }

    /**
     * Catmull-Rom style conversion of a polyline into a continuous cubic path.
     * No straight segments at corners — every turn is a smooth cubic.
     */
    private fun smoothCubicThrough(pts: List<PointF>): Path {
        val p = Path()
        ...
        // For each interior segment use cubic with controls pulled toward
        // neighboring points (tension ~0.5 → soft pipe curves).
        val tension = 0.55f
